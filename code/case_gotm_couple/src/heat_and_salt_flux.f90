...
! This part deals with fluxes of heat (W/m^2) and salt (mm/hr) at the surface
...
! ----------------------------------
  ...
  do j=1,NJ 
    do i=1,NI 
      dfactor=fac*Jac(i,j,NK)*wz(i,j,NK)
...
!     ref_salt=33.d0 
!     precip_flux= ref_salt*rainrate(j)*1d-3/3600.d0 

      Kdfluxdzt = DL*(evap_flux-precip_flux)
      Sdif(i,j,NK) =dfactor*Kdfluxdzt
